...
public class Boat extends AnimatedSprite{
	...
	public Boat(float pX, float pY, ITiledTextureRegion pTiledTextureRegion,
			VertexBufferObjectManager pVertexBufferObjectManager, FishingHut sparent) {
...
public void movementBoat(){
	...
	timer = 250;}
	if(MainActivity.calculateDistance(this, target)<48){
		checkCollision();
	}
...
private Sprite findSpot(){
	...
	int pos = -1;
	for (int i =0; i<MainActivity.fishspots.size();i++){
		if(MainActivity.fishspots.get(i).gotSpace())
			if(MainActivity.calculateDistance(this, MainActivity.fishspots.get(i))<distance){
					pos = i;}
		
	}
	if(pos!=-1)
		{ res = MainActivity.fishspots.get(pos);
		res.boats++;
		return res;}
	else
	return parent;
